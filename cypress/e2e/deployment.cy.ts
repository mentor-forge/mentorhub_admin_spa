/**
 * Deployment-boundary coverage for the `/admin/` nginx + Vite base model.
 *
 ...
 * `/admin/api/*` reaches admin_api (including the regex-vs-prefix precedence edge).
 */
describe('Deployment boundary (/admin/ nginx)', () => {
  ...
  it('redirects / and /admin to /admin/', () => {
    ...
    cy.request({ url: '/admin', followRedirect: false }).then((response) => {
      expect(response.status).to.be.oneOf([301, 302])
      expect(response.headers.location).to.equal('/admin/')
    })
  })

  it('serves the Admin SPA shell at /admin/ with no-store (not immutable)', () => {
    cy.request('/admin/').then((response) => {
      expect(response.status).to.equal(200)
      expect(contentType(response.headers)).to.include('text/html')
      expect(String(response.body)).to.include('Admin')
      expect(String(response.body)).to.include('/admin/runtime-config.js')
      expect(String(response.body)).to.match(/\/admin\/assets\/.+\.js/)
  ...
  it('falls back history routes under /admin/ to the SPA document with no-store', () => {
    cy.request('/admin/settings/does-not-exist-yet').then((response) => {
      expect(response.status).to.equal(200)
      expect(contentType(response.headers)).to.include('text/html')
      expect(String(response.body)).to.include('/admin/runtime-config.js')
  ...
  it('serves versioned /admin/assets with public, immutable caching', () => {
    cy.request('/admin/').then((shell) => {
      const match = /\/admin\/assets\/[^"']+\.js/.exec(String(shell.body))
  ...
  it('serves /admin/runtime-config.js with no-store and the Admin IdP URI', () => {
    cy.request('/admin/runtime-config.js').then((prefixed) => {
      ...
      // serve another journey's root-level runtime-config when the browser asks for /admin/.
  ...
  it('does not let the static-asset regex swallow /admin/api/*.js', () => {
    // Without `^~` on `/admin/api/`, nginx would evaluate the `.js` asset regex and miss
    // the API proxy. A proxied miss comes from admin_api (JSON or its own 404 page),
    // never the Admin SPA shell or an immutable asset response.
    cy.request({
      url: '/admin/api/definitely-not-a-static-asset.js',
      failOnStatusCode: false,
    }).then((response) => {
      expect(String(response.body)).not.to.include('/admin/runtime-config.js')
      expect(String(response.body)).not.to.include('/admin/assets/')
      expect(String(response.body)).not.to.include('<title>Admin</title>')
      expect(cacheControl(response.headers)).not.to.include('immutable')
      expect(response.status).to.be.oneOf([401, 403, 404])
    })
  })

  it('returns an API authorization failure (not the SPA) for unauthenticated /admin/api', () => {
    cy.request({
  ...
  it('proxies an authenticated admin Bearer request through /admin/api to admin_api', () => {
  ...
  it('denies a least-privileged mentee token on admin-only /admin/api/setting', () => {
    // UI gating is UX; API authorization is security. Prove the packaged proxy still
    // reaches admin_api RBAC for a non-admin identity.
    cy.task<{ token: string }>('signCypressJwt', {
      roles: ['mentee'],
      secret: Cypress.env('JWT_SECRET'),
    }).then(({ token }) => {
